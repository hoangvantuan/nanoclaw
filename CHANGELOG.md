...
- [BREAKING] **All providers now share `memory/`, while standing role and persona live in `instructions.prepend.md`.** Session-start hooks load `memory/index.md` and `memory/system/definition.md` on startup, clear, and compact, but skip resume. Normal startup never imports legacy `.seed.md`, `CLAUDE.local.md`, or `imported-agent-memory.md`. **Migration:** run `/migrate-memory` before messaging affected groups; it quiesces them, moves legacy files without reading or overwriting them, and has the group agent distill their content inside the container. See [docs/provider-migration.md](docs/provider-migration.md).
