...
describe('telegram-topic-support: task session routing', () => {
  it('inherits the origin topic routing for a task session', () => {
    ...
    tdb
      ...
      .run('task-1', 'task', now(), JSON.stringify({ prompt: 'daily', script: null, originSessionId: origin.id }));
    tdb.close();
  ...
  it('writes null routing for a task session with no origin (CLI-created)', () => {
    ...
    tdb
      .prepare('INSERT INTO messages_in (id, kind, timestamp, content) VALUES (?, ?, ?, ?)')
      .run('task-1', 'task', now(), JSON.stringify({ prompt: 'daily', script: null, originSessionId: null }));
    tdb.close();
